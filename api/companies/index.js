...
module.exports = async function (context, req) {
  context.log('Companies function processed a request.');
  ...
  try {
    ...
    if (req.method === 'GET') {
      ...
      context.res = {
        ...
        body: resources.length > 0 ? resources[0].companies || [] : []
      };
      ...
      if (!Array.isArray(companies)) {
        context.res = {
          status: 400,
          headers: { 'Content-Type': 'application/json' },
          body: { error: 'companies must be an array' }
        };
        return;
      }
    ...
    context.log.error('Error processing companies:', error);
    context.res = {
      status: 500,
      headers: { 'Content-Type': 'application/json' },
      body: { error: 'Failed to process companies', message: error.message }
    };
  }
};
